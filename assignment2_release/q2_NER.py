...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder=tf.placeholder(tf.int32,shape=(None,self.config.window_size),name='inputs')
    self.labels_placeholder=tf.placeholder(tf.float32,shape=(None,self.config.label_size),name='targets')
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      L=tf.get_variable(shape=[len(self.wv),self.config.embed_size],name='embed')
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('layer1',initializer=xavier_weight_init()) as scope:
      ...
      if (self.config.l2):
        tf.add_to_collection('loss',0.5*self.config.l2*tf.nn.l2_loss(W))
    ...
    with tf.variable_scope('layer2',initializer=xavier_weight_init()) as scope:
      ...
      outs=tf.matmul(h,U)+b2
      if (self.config.l2):
        tf.add_to_collection('loss',0.5*self.config.l2*tf.nn.l2_loss(U))
      output=tf.nn.dropout(outs,self.dropout_placeholder)
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    cross_entropy=tf.nn.softmax_cross_entropy_with_logits(y,self.labels_placeholder)
    cross_entropy_loss=tf.reduce_mean(cross_entropy)
    loss=cross_entropy_loss+tf.add_n(tf.get_collection('loss'))
    ### END YOUR CODE
    return loss
