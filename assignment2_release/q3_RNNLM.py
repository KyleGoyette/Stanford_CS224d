...
class RNNLM_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder=tf.placeholder(tf.int32,name='inputs',shape=[None,self.config.num_steps])
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      inputs=[]
      L=tf.get_variable("embed",shape=[len(self.vocab),self.config.embed_size])

      curr_inps = tf.split(1,self.config.num_steps,self.input_placeholder)
      for inp in curr_inps:
        inputs.append(tf.squeeze(tf.nn.embedding_lookup(L,inp),[1]))
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('projection'):
      U=tf.get_variable("U_matrix",shape=[self.config.hidden_size,len(self.vocab)])
      b_2=tf.get_variable("bias_proj",shape=[len(self.vocab)])
      outputs=[tf.matmul(outs,U)+b_2 for outs in rnn_outputs]
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    ones=[tf.ones([self.config.num_steps*self.config.batch_size])]
    loss=sequence_loss([output],[tf.reshape(self.labels_placeholder,[-1])],[tf.ones([self.config.num_steps*self.config.batch_size])])
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN') as scope:
      ...
      for step,current_input in enumerate(inputs):
        ...
        Wh=tf.get_variable('Wh',[self.config.hidden_size,self.config.hidden_size])
        Wx=tf.get_variable('Wx',[self.config.embed_size,self.config.hidden_size])
        b_1=tf.get_variable('b_1',[self.config.hidden_size])
        curr_state=tf.nn.sigmoid(tf.matmul(curr_state,Wh)+tf.matmul(current_input,Wx)+b_1)
        rnn_outputs.append(curr_state)
      self.final_state=rnn_outputs[step]
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ...
    curr_state,y_pred=session.run([model.final_state,model.predictions[-1]],feed_dict=feed_dict)
    ### END YOUR CODE
