...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    sig=sigmoid(np.dot(predicted,outputVectors[target,:]))
    cost = -np.log(sig)
    delta=sig-1
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted = inputVectors[curr_index,:]

    cost=0.0
    gradIn=np.zeros(inputVectors.shape)
    gradOut = np.zeros(outputVectors.shape)
    gradIn=np.matrix(gradIn)
    for word in contextWords:
        index=tokens[word]
        cost_temp, grad_pred_temp, grad_temp = word2vecCostAndGradient(predicted, index,outputVectors,dataset)
        cost+=cost_temp
        gradOut+=grad_temp
        gradIn[curr_index,:]+=grad_pred_temp
    ### END YOUR CODE
