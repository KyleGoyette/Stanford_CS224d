...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ### YOUR CODE HERE
            tf.get_variable("Embed_Matrix", shape = [self.vocab.total_words, self.config.embed_size])
            tf.get_variable("W1_matrix", shape = [2*self.config.embed_size,self.config.embed_size])
    ...
    def add_model(self, node):
        ...
        with tf.variable_scope('Composition', reuse=True):
            ### YOUR CODE HERE
            embedding = tf.get_variable("Embed_Matrix")
            W1 = tf.get_variable("W1_matrix")
        ...
        if node.isLeaf:
            ...
            curr_word_id = self.vocab.encode(node.word)
            curr_node_tensor = embedding[curr_word_id,:]
            ...
            ### YOUR CODE HERE
            children_tensors = tf.concat(1,[node_tensors[node.left], node_tensors[node.right]])
            curr_node_tensor = tf.nn.relu(tf.matmul(children_tensors,W1)+b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        loss = None
        # YOUR CODE HERE
        data_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits,labels, name="Data_Loss"))

        with tf.variable_scope("Composition",reuse=True):
            W = tf.get_variable("W1_matrix")
        with tf.variable_scope("Projection",reuse=True):
            U = tf.get_variable("U")

        reg_loss=tf.nn.l2_loss(W)+tf.nn.l2_loss(U)

        loss=data_loss+self.config.l2*reg_loss
